<script>
	...
	import {clearFlock, drawFlock} from "$lib/utils/genuary.2024.19.js";
    ...
    let sketches = [
		...
		{name: "19. Flocking", sketch: drawFlock, reset: clearFlock, animation: true, md: "/md/19.md"}
    ];
...
<main>
	<h1>
		<a
			...
			target="_blank"
			data-umami-event="follow-link"
			data-umami-event-link="genuary.2024">Genuary 2024</a
		>
	</h1>
	<h2>
		entries by <a
			...
			target="_self"
			data-umami-event="follow-link"
			data-umami-event-link="d17e.dev">d17e.dev</a
		>
	</h2>

	<div class="header">
		<select
			on:change={handleSelectSketch}
			id="sketch-selection"
			aria-label="select prompt from list"
		...
		<button
			class="dice-button"
			on:click={handleDice}
			title="reset the sketch"
			data-umami-event="sketch-refresh"
			data-umami-event-sketch={sketches[selectedSketchIndex].name}
		>
			<svg
				class="dice-svg"
		...
		<button
			class="dice-button second"
			on:click={handleDownload}
			title="download frame"
			data-umami-event="sketch-download-frame"
			data-umami-event-sketch={sketches[selectedSketchIndex].name}
		>
			<svg
				class="dice-svg"
		...
		{#if isLocalHost}
			<button
				...
				data-umami-event="sketch-record"
				data-umami-event-sketch={sketches[selectedSketchIndex].name}
			>
				<svg
					class="dice-svg"
	...
	<div class="footer">
		<div class="debug-container">
			...
			<input
				...
				id="debug"
				name="debug"
				on:change={handleDebug}
				bind:checked={debug}
				data-umami-event="check-debug"
				data-umami-event-checked={debug}
				data-umami-event-sketch={sketches[selectedSketchIndex].name}
			/>
		</div>
	</div>
