...
function createBaseShape(paper, topLeftPoint, size, palette, opacity, blendMode) {
	...
	switch (shapeSelect) {
		case 1:
			path = new paper.Path({
				...
				strokeCap: 'round',
				fillColor: buildGradientForPalette(
					palette,
					topLeftPoint,
					new paper.Point(topLeftPoint.x + size, topLeftPoint.y + size)
				),
	...
	if (path) {
		path.rotate(
			getRandomInt(0, 3) * 90,
			new paper.Point(topLeftPoint.x + size / 2, topLeftPoint.y + size / 2)
		);
	}
	return path;
}
...
export function drawGenerativeColors(paper, event, debug) {
	...
	const canvasOffset =
		(Math.max(paper.view.bounds.width, paper.view.bounds.height) - canvasSize) / 2;

	const blendMode = ['multiply', 'screen', 'overlay', 'soft-light', 'hard-light'][
		getRandomInt(0, 4)
	];
	...
	for (let i = 0; i < LAYERS; i++) {
		...
		if (blocks === 1) {
			shapes.push(
				createBaseShape(
					paper,
					new paper.Point(canvasOffset, canvasOffset),
					blockSize,
					colors.baseSet,
					opacity,
					blendMode
			...
			for (let j = 0; j < layerBlocks; j++) {
				...
				let center = new paper.Point(
					canvasOffset + blockSize / 2 + column * blockSize,
					canvasOffset + blockSize / 2 + row * blockSize
				);
				let color = buildGradientForPalette(
					colors.contrastSet,
					new paper.Point(canvasOffset, canvasOffset)
				);
