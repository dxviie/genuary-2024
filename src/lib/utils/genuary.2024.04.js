...
export function drawPixels(paper, event, debug) {
	...
	screen.pixels
		.filter((pixel) => pixel.group.opacity > 0.12)
		.forEach((pixel) => {
	...
	for (let i = 0; i < tracer.length; i++) {
		...
		let upper = Math.floor(
			((block.bounds.y + block.bounds.height - canvasOffset) / canvasSize) * PIXEL_DIM
		);
		...
		if (pixels[lower]) {
			...
			pixels[lower].setGlow(new paper.Color(`rgb(${red}, ${green}, ${blue})`));
		...
		if (pixels[upper]) {
			...
			pixels[upper].setGlow(new paper.Color('rgb(255, 255, 255)'));
...
function createCRTScreen(paper, canvasOffset, pixelSize, shift) {
	...
	for (let x = 0; x < PIXEL_DIM; x++) {
		...
		for (let y = 0; y < PIXEL_DIM; y++) {
			let pixel = createCRTPixel(
				paper,
				canvasOffset + x * pixelSize,
				canvasOffset + y * pixelSize,
				pixelSize
			);
