...
export function drawDroste(paper, event, debug) {
    ...
    const drosteScaleCenter = new paper.Point((drosteQuadrant === 0 || drosteQuadrant === 2) ? 0 : canvasSize,
...
function createDrosteGroup(paper, quadrant, quadrantSize, debug) {
    ...
    for (let q = 0; q < 4; q++) {
        ...
        if (quadrant === q) {
            ...
            let particle = createParticle(paper, topLeft.add(new paper.Point(quadrantSize/2, quadrantSize/2)), quadrantSize, 'rgb(0, 0, 0)', 'normal', 1);
